...
Future<Stream<CommissionReportModel>> getCommissionHistory() async {
  Uri uri = Helper.getUri('Api_admin/commissionReport/list/${currentUser.value.id}');
  print(uri);
  Map<String, dynamic> _queryParams = {};
  ...
  try {
    ...
    return streamedRest.stream
        ...
        .map((data) => Helper.getData(data))
        .expand((data) => (data as List))
        .map((data) => CommissionReportModel.fromJSON(data));
  } catch (e) {

    print(e);
    print(CustomTrace(StackTrace.current, message: uri.toString()).toString());
    return new Stream.value(new CommissionReportModel.fromJSON({}));
  }
}

Future<Stream<ShopRatingModels>> getShopReviewlist(id) async {
  ...
  try {
    ...
    return streamedRest.stream
        ...
        .map((data) =>  Helper.getData(data))
        .expand((data) => (data as List))
        .map((data) => ShopRatingModels.fromJSON(data));
  } catch (e) {
    print(CustomTrace(StackTrace.current, message: uri.toString()).toString());
    return new Stream.value(new ShopRatingModels.fromJSON({}));
  }
}

Future<Stream<VendorPlanHistory>> getMembershipPlanHistory() async
{
  Uri uri = Helper.getUri('Api_admin/userMembershipHistory');
  print(uri);
  Map<String, dynamic> _queryParams = {};
...
Future<ProfileResponseNew> getProfileDetailsById(int shopId) async {
  Uri uri = Helper.getUri('Api_vendor/profileUpdate/profilelist/$shopId');
  ...
  if (response.statusCode == 200) {
    ...
    if (response.statusCode == 200) {
      ...
      res = ProfileResponseNew.fromJSON(json.decode(response.body)['data']);
    } else {
      throw new Exception(response.body);
    }
...
Future<bool> updateVendorProfile(dataValue, type,id) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  // ignore: deprecated_member_use
  final String url = '${GlobalConfiguration().getString('api_base_url')}api_vendor/profileUpdate/$type/$id?$_apiToken';
  print(url);
  print(json.encode(dataValue.toMap()));
  bool res;
  final client = new http.Client();
  final response = await client.post(
    Uri.parse(url),
...
Future<RoleResponse> listAdminRole() async {

  // ignore: deprecated_member_use
  final String url = '${GlobalConfiguration().getString('api_base_url')}api_admin/listRoles';
  print(url);
  ...
  if (response.statusCode == 200) {
    roleResponse = RoleResponse.fromJson(json.decode(response.body));
...
Future<bool> addAdminRole(RoleRegisterModel registerModel) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  // ignore: deprecated_member_use
  final String url = '${GlobalConfiguration().getString('api_base_url')}api_admin/saveAdminZone';
  print(url);
...
Future<bool> deleteAdminRole(id) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  // ignore: deprecated_member_use
  final String url = '${GlobalConfiguration().getString('api_base_url')}api_admin/deleteAdmin/$id';
  print(url);
