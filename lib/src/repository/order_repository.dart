...
Future<Stream<OrderList>> getOrderList(String status) async {
  ...
  Uri uri = Helper.getUri(
      'Api_admin/Order/list/${currentUser.value.id}/$status/${currentUser.value.id}/${currentUser.value.role}/${currentUser.value.zoneId}');
...
Future<Stream<Map<String, dynamic>>> getInvoiceReport(shopId) async {
  ...
  print('Api_admin/payment/paymentlist/$shopId');
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', uri));
    return streamedRest.stream
...
Future<InvoiceModel> getInvoice(orderID) async {
  ...
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_vendor/Order/invoicefull/$orderID';
  InvoiceModel res;
  final client = new http.Client();
...
Future<Checkout> getInvoiceDetails(orderID) async {
  // ignore: deprecated_member_use
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_vendor/Order/invoiceView/$orderID';
...
editOrderStatusDetails(OrderList data, id) async {
  // ignore: deprecated_member_use
  Uri uri = Helper.getUri(
      'Api_admin/Order/editstatus/$id/null/${currentUser.value.id}');
...
Future<SummaryReport> getSummaryReport(vendorID) async {
  // ignore: deprecated_member_use
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_admin/payment/summaryReport/$vendorID';
...
Future<DriverStatus> manualStatusUpdate(orderID, id) async {
  // ignore: deprecated_member_use
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_vendor/orderAssign/update/$id/$orderID';
...
// ignore: missing_return
Future<DriverStatus> orderStatusUpdateStep2(
    type, driverState, orderID, vendorId) async {
  final String _apiToken = 'api_token=${currentUser.value.apiToken}';
  // ignore: deprecated_member_use
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_admin/orderAssign/$type/$driverState/$orderID/${currentUser.value.id}/$vendorId?$_apiToken';
  print(url);
  final client = new http.Client();
  final response = await client.post(
    Uri.parse(url),
...
Future<bool> orderStatusUpdate(orderID, status) async {
  ...
  // ignore: deprecated_member_use
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}Api_admin/Order/statusUpdate/$orderID/$status/${currentUser.value.id}?$_apiToken';
...
Future<Stream<DropDownModel>> getDriverData(
    type, driverState, orderID, vendorId) async {
  Uri uri = Helper.getUri(
      'Api_admin/orderAssign/$type/$driverState/$orderID/${currentUser.value.id}/$vendorId');
  Map<String, dynamic> _queryParams = {};
  print(uri);
...
Future<Stream<OrderList>> getOrderTakeawayList(String status) async {
  Uri uri = Helper.getUri(
      'Api_admin/takeawayOrder/list/$status/null/${currentUser.value.id}');
  Map<String, dynamic> _queryParams = {};
