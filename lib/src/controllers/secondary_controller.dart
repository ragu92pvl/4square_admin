...
import 'package:login_and_signup_web/src/models/othersevice_delivery_Fess.dart';
...
class SecondaryController extends ControllerMVC {
  ...
  OtherServiceDeliveryFees otherServiceDeliveryFees =
      new OtherServiceDeliveryFees();

  CarTypeModel carTypeData = CarTypeModel();
  Currency currencyData = Currency();
  GlobalKey<FormState> deliveryFeesFormKey;
  GlobalKey<FormState> otherservicedeliveryFeesFormKey;
  ...
  List<Map<String, dynamic>> settlementSource = List<Map<String, dynamic>>();
  List<Map<String, dynamic>> settlementCompleteSource =
      List<Map<String, dynamic>>();
  // ignore: deprecated_member_use
  List<Map<String, dynamic>> originalSource = List<Map<String, dynamic>>();
  List<Map<String, dynamic>> originalSettlementSource =
      List<Map<String, dynamic>>();
  List<Map<String, dynamic>> originalSettlementCompleteSource =
      List<Map<String, dynamic>>();
  bool isLoading = true;
  ...
  List<OtherServiceDeliveryFees> otherservicedeliveryFeesList =
  ...
  SecondaryController() {
    ...
    otherservicedeliveryFeesFormKey = new GlobalKey<FormState>();
  ...
  addEdCoupon(context, id, pageType) {
    if (registerFormKey.currentState.validate()) {
      ...
      Overlay.of(context).insert(loader);
      repository.addECoupon(couponData, pageType).then((value) {
        showToastPopup("Update Successfully", context,
            gravity: Toast.BOTTOM, duration: Toast.LENGTH_SHORT);
      }).catchError((e) {
        print(e);
  ...
  addEdFocusType(context, id, pageType) {
    if (generalFormKey.currentState.validate()) {
      ...
      if ((shopTypeData.previewImage != null &&
          shopTypeData.coverImage != null)) {
        Overlay.of(context).insert(loader);
  ...
  Future<void> addEditProvider(paraType, id, context) async {
    if (registerFormKey.currentState.validate()) {
      ...
      if (providerData.uploadImage != null) {
        ...
        showToastPopup("Please Upload Image", context,
            gravity: Toast.BOTTOM, duration: Toast.LENGTH_SHORT);
      }
    }
  }
  ...
  Future<void> addEditMemberPlanList(paraType, id, context) async {
    if (registerFormKey.currentState.validate()) {
      ...
      if (vendorMembershipData.uploadImage != null) {
        Overlay.of(context).insert(loader);
        await repository
            .aEVendorMembership(vendorMembershipData, paraType, id)
            .then((value) {
          showToastPopup("Update Successfully", context,
              gravity: Toast.BOTTOM, duration: Toast.LENGTH_SHORT);
        }).catchError((e) {
  ...
  otherservicedeliveryFeesUpdate(context, id, pageType) {
    if (otherservicedeliveryFeesFormKey.currentState.validate()) {
      otherservicedeliveryFeesFormKey.currentState.save();
      Overlay.of(context).insert(loader);
      setState(() => otherservicedeliveryFeesList.clear());

      repository
          .addEdOtherDeliveryFees(
        otherServiceDeliveryFees,
        pageType,
        id,
      )
        ...
        Navigator.pop(context);
        listenForOtherserviceDeliveryFees();
        Helper.hideLoader(loader);
      });
    }
  }
  ...
  walletStatsUpdate(type, invoiceId, id, amount, pagetype) async {
    Overlay.of(context).insert(loader);
    await repository
        .addwalletStatusUpdate(type, invoiceId, id, amount, pagetype)
        .then((value) {
      showToast("Update Successfully",
          gravity: Toast.BOTTOM, duration: Toast.LENGTH_SHORT);
    }).catchError((e) {
  ...
  Future<void> listenForOtherserviceDeliveryFees() async {
    final Stream<OtherServiceDeliveryFees> stream =
        await getOtherserviceDeliveryFees();
    stream.listen((OtherServiceDeliveryFees _list) {
      setState(() => otherservicedeliveryFeesList.add(_list));
    }, onError: (a) {
      print(a);
    }, onDone: () {});
  }

  Future<void> listenForVendorSettlementList(shopId) async {
    setState(() => isLoading = true);
    final Stream<Map<String, dynamic>> stream =
        await getVendorSettlementList(shopId);
    print(stream);
    stream.listen((Map<String, dynamic> _list) {
      print(_list.toString());
      setState(() => settlementSource.add(_list));
      setState(() => originalSettlementSource.add(_list));
      setState(() => isLoading = false);
    }, onError: (a) {
      print(a);
    }, onDone: () {});
  ...
  Future<void> listenForVendorSettlementCompleteList(shopId) async {
    setState(() => isLoading = true);
    final Stream<Map<String, dynamic>> stream =
        await getVendorSettlementCompleteList(shopId);
    print(stream);
    stream.listen((Map<String, dynamic> _list) {
      print(_list.toString());
      setState(() => settlementCompleteSource.add(_list));
      setState(() => originalSettlementCompleteSource.add(_list));
      setState(() => isLoading = false);
    }, onError: (a) {
      print(a);
    }, onDone: () {});
  ...
  Future<void> changeSettlement(
      fromDate, lastDate, int vendorId, transactionId) async {
    print("$fromDate-$lastDate");
    Overlay.of(context).insert(loader);
    await repository
        .changeSettlementStatus(fromDate, lastDate, vendorId, transactionId)
        .then((value) {
      if (value) {
        settlementSource.clear();
        originalSettlementSource.clear();
